...
void SimpleTimer::getStatus(Print* output) {
    for (int i=0; i<MAX_TIMERS; i++) {
        // only process active timers
        if (callbacks[i] && enabled[i]) {
		output->print(i);
		output->print(":");
//		output->print(callbacks[i]);
		output->print(" Delays:");
		output->print(delays[i]);
		output->print(" MaxNumRuns: ");
		output->print(maxNumRuns[i]);
		output->print(" NumRuns:");
		output->print(numRuns[i]);
		output->print(" Next in ms:");
		long nextMillis=prev_millis[i]+delays[i]-millis();
		output->println(nextMillis);
	}
   }
}
